<?php
	...
	class BootstrapFormHelper extends FormHelper {
		 ...
		 * @return void
		 */
		private function _parseInputOptions($field, $options = array()) {
			if (is_array($field)) {
				$options = $field;
			} else {
				$options['field'] = $field;
		 ...
		 * @return void
		 */
		private function _constructLabel($options, $basic = true) {
		 ...
		 * @return void
		 */
		private function _constructInput($options) {
			if (in_array($options['type'], array('checkbox'))) {
				$options['input'] = '';
			}
		 ...
		 * @return void
		 */
		private function _constuctInputAndAddon($options) {
			if (isset($options['input'])) { return $options['input']; }
		 ...
		 * @return void
		 */
		private function _handleInputAddon($options) {
			$input = $options['input'];
			if ($options['append']) {
		...
		public function input($field, $options = array()) {
			if(isset($options['cake']) && $options['cake']) {
				unset($options['cake']);
			...
			if(isset($opt['cake']) && $opt['cake']) {
				unset($opt['cake']);
				return parent::input($options['field'], $opt);
			}
